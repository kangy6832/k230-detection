...
# 布局示意：
#   ┌──────┬──────────────┬──────┐
#   │  0   │  1  2  3     │  4   │  顶部行 (y=0~160)
#   ├──────┼──────────────┼──────┤
#   │5 6 7 │     8        │9 10  │  中部行 (y=160~320)
#   │      │              │11    │
#   ├──────┼──────────────┼──────┤
#   │ 12   │ 14  15  13   │ 16   │  底部行 (y=320~480)
#   └──────┴──────────────┴──────┘
ROIS = [ # [ROI, weight]
        # 顶部行 (y=0~160)
        (0,   0, 213, 160, 0.1),  # idx 0  左上
        
        (213, 0, 213, 40, 0.1),   # idx 1  中上-上
        ...
        (586, 160, 54, 160, 0.1), # idx 11 中右-右
        ...
        (213, 440, 213, 40, 0.1), # idx 15 中下-下
...
ZONE_LEFT   = [0, 5, 6, 12]     # 左侧列     —— 检测画面左侧的垂直线段
ZONE_RIGHT  = [4, 10, 11, 16]   # 右侧列     —— 检测画面右侧的垂直线段
ZONE_CENTER = [7, 8, 9]         # 中心区域   —— 循线核心区域，判断线条是否居中
...
try:
    ...
    while True:
        ...
        if state == STATE_FOLLOWING:
            ...
            # 拐角检测：前方有线（还在沿当前方向走）时，
            # 检查侧方是否也出现了线（拐角出现的信号）
            # 冷却期内跳过拐角检测，防止旧线段误触发方向反转
            detected = -1  # 检测到的拐角方向，-1 表示未检测到拐角

            if forward and cooldown_frames == 0:
                # 根据当前运动方向，检查左侧或右侧是否出现线条
                # prev_dir 排除旧线段所在区域，防止机器人回头走向来时的方向
                if move_dir == DIR_UP:
                    # 向上走时：前方=上区，侧方=左区/右区
                    if prev_dir != DIR_LEFT and left:    detected = DIR_LEFT    # 左侧有线且非旧方向 → 左拐
                    elif prev_dir != DIR_RIGHT and right: detected = DIR_RIGHT   # 右侧有线且非旧方向 → 右拐
                    ...
                    # 向左走时：前方=左区，侧方=上区/下区
                    if prev_dir != DIR_UP and top:        detected = DIR_UP      # 上方有线且非旧方向 → 上拐
                    elif prev_dir != DIR_DOWN and bottom:  detected = DIR_DOWN    # 下方有线且非旧方向 → 下拐
        ...
        print("S:%s D:%s X:%s%.4f Y:%s%.4f fps:%.1f" % (
            ...
            x_sign, abs(target[0]), y_sign, abs(target[1]), clock.fps()))
