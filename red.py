...
RED_THRESHOLD = (23, 47, 15, 65, 13, 55)
MIN_RED_PIXELS = 80
MIN_RED_AREA = 80
RED_CUBE_SIZE_MM = 22.0
...
def estimate_marker_pose(corners):
    ...
    for world_point in world_points:
        corner_cams.append(transform_point_to_camera(rmat, tvec, (world_point[0], world_point[1], 0.0)))

    return {
        "valid": True,
        "target_type": TARGET_TYPE_RECT,
        "corner_images": corners,
        "target_points": corner_cams,
    }


def estimate_red_pose(blob):
    x, y, w, h = blob.rect()
    pixel_size = max(w, h)
    if pixel_size <= 0:
        return {"valid": False, "reason": "invalid red size"}

    center_u = x + w // 2
    center_v = y + h // 2
    z = FX * RED_CUBE_SIZE_MM / float(pixel_size)
    cam_x = (float(center_u) - CX) * z / FX
    cam_y = (float(center_v) - CY) * z / FY
    return {
        "valid": True,
        "target_type": TARGET_TYPE_RED,
        "center_image": (center_u, center_v),
        "target_cam": (cam_x, cam_y, z),
        "target_points": [(cam_x, cam_y, z)],
        "rect": (x, y, w, h),
    }


def find_red_blob(img):
    blobs = img.find_blobs([RED_THRESHOLD], pixels_threshold=MIN_RED_PIXELS, area_threshold=MIN_RED_AREA, merge=True)
    best_blob = None
    best_pixels = 0
...
def draw_red_status(img, pose):
    x, y, w, h = pose["rect"]
    center_u, center_v = pose["center_image"]
    img.draw_rectangle(x, y, w, h, color=(0, 255, 0), thickness=2)
    img.draw_cross(center_u, center_v, color=(255, 255, 255), size=8, thickness=2)

    cam_x, cam_y, cam_z = pose["target_cam"]
    img.draw_string(2, 2, "red_cam_mm=({:.1f}, {:.1f}, {:.1f})".format(cam_x, cam_y, cam_z), color=(255, 255, 255), scale=1)
...
def draw_status(img, corners, pose):
    ...
    for corner_u, corner_v in pose["corner_images"]:
        img.draw_circle(corner_u, corner_v, 4, color=(0, 255, 0), fill=True)

    right_top_index = get_right_top_corner_index(pose["corner_images"])
    corner_u, corner_v = pose["corner_images"][right_top_index]
    img.draw_circle(corner_u, corner_v, 6, color=(255, 255, 0), fill=True)

    right_top_x, right_top_y, right_top_z = pose["target_points"][right_top_index]
    img.draw_string(2, 2, "corner_rt_mm=({:.1f}, {:.1f}, {:.1f})".format(right_top_x, right_top_y, right_top_z), color=(255, 255, 255), scale=1)
...
def send_target_to_stm32(pose):
    ...
    target_type = pose["target_type"]
    points = list(pose["target_points"])
    while len(points) < POINTS_PER_PACKET:
        points.append((0.0, 0.0, 0.0))

    payload = [target_type]
    for x, y, z in points[:POINTS_PER_PACKET]:
        payload.extend([float(x), float(y), float(z)])

    packet = struct.pack("<i12f", *payload)
    uart.write(packet)
    return True
...
def print_detection_result(pose, sent):
    ...
    if pose["target_type"] == TARGET_TYPE_RED:
        x, y, z = pose["target_points"][0]
        print("red_cam_mm=({:.1f}, {:.1f}, {:.1f})".format(x, y, z))
    else:
        labels = ("lt", "rt", "rb", "lb")
        parts = []
        for label, point in zip(labels, pose["target_points"]):
            x, y, z = point
            parts.append("{}=({:.1f}, {:.1f}, {:.1f})".format(label, x, y, z))
        print("rect_cam_mm " + " ".join(parts))
...
try:
    ...
    red_seen_count = 0
    red_lost_count = RED_LOST_FRAMES
    ...
    while True:
        ...
        red_blob = find_red_blob(img)
        target_pose = None

        if red_blob:
            red_seen_count += 1
            red_lost_count = 0
            if red_seen_count >= RED_CONFIRM_FRAMES:
                current_mode = MODE_RED
        else:
            red_seen_count = 0
            red_lost_count += 1
            if red_lost_count >= RED_LOST_FRAMES:
                current_mode = MODE_RECT

        if current_mode == MODE_RED and red_blob:
            target_pose = estimate_red_pose(red_blob)
            if target_pose["valid"]:
                draw_red_status(img, target_pose)
        elif current_mode == MODE_RECT and not red_blob:
